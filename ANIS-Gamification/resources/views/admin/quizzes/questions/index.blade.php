        ...
        <div class="grid gap-8 lg:grid-cols-[260px_minmax(0,1fr)]">
            ...
            <div class="space-y-8">
                ...
                <header class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <p class="text-sm text-on-surface-variant">Admin • Questions du quiz</p>
                        <h1 class="text-3xl font-headline font-extrabold text-on-surface">{{ $quiz->title }}</h1>
                        <p class="text-sm text-on-surface-variant mt-1">{{ $quiz->description }}</p>
                    </div>
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
                        ...
                        <a href="{{ route('admin.quizzes.questions.create', $quiz) }}" class="inline-flex items-center gap-2 rounded-full bg-primary px-5 py-3 text-sm font-semibold text-on-primary hover:bg-primary-dim transition">
                                ...
                                @forelse($quiz->questions as $question)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium flex flex-wrap justify-end gap-2">
                                            <a href="{{ route('admin.quizzes.questions.edit', [$quiz, $question]) }}" class="inline-flex items-center gap-2 rounded-full border border-primary/10 bg-primary/5 px-3 py-2 text-primary hover:bg-primary/10 transition">
                                                <span class="material-symbols-outlined text-base">edit</span>
                                                Modifier
                                            </a>
                                            <form action="{{ route('admin.quizzes.questions.destroy', [$quiz, $question]) }}" method="POST" class="inline-block">
                                                @csrf
                                                @method('DELETE')
