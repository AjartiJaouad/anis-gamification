...
<head>
    ...
    <title>Admin - Ajouter une question QCM</title>
        ...
        <div class="grid gap-8 lg:grid-cols-[260px_minmax(0,1fr)]">
            ...
            <div class="space-y-8">
                <header class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <p class="text-sm text-on-surface-variant">Admin • Ajouter une question</p>
                        <h1 class="text-3xl font-headline font-extrabold text-on-surface">Question pour : {{ $quiz->title }}</h1>
                    </div>
                    ...
                    <form action="{{ route('admin.quizzes.questions.store', $quiz) }}" method="POST" class="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea id="question" name="question" rows="4" required
                                class="mt-2 w-full rounded-2xl border border-surface-container bg-surface-container-lowest px-4 py-3 text-on-surface outline-none focus:border-primary focus:ring-2 focus:ring-primary/10">{{ old('question') }}</textarea>
                            @error('question')
                                <p class="mt-2 text-sm text-error">{{ $message }}</p>
                            @enderror
                        </div>
                            ...
                            @for($i = 0; $i < 4; $i++)
                                <div class="rounded-3xl border border-surface-container p-5 bg-surface-container-lowest">
                                    <div class="flex items-center justify-between gap-4">
                                        <label for="option_{{ $i }}" class="text-sm font-semibold text-on-surface">Réponse {{ $i + 1 }}</label>
                                        <label class="inline-flex items-center gap-2 text-sm text-on-surface-variant">
                                            <input type="checkbox" name="correct_options[]" value="{{ $i }}" {{ is_array(old('correct_options')) && in_array($i, old('correct_options')) ? 'checked' : '' }} class="h-4 w-4 rounded border-surface-container text-primary focus:ring-primary" />
                                            Correcte
                                        </label>
                                    </div>
                                    <input id="option_{{ $i }}" name="options[{{ $i }}]" type="text" value="{{ old('options.' . $i) }}" required
