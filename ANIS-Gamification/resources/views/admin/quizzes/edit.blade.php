...
<head>
    ...
    <title>Admin - Modifier le quiz</title>
        ...
        <div class="grid gap-8 lg:grid-cols-[260px_minmax(0,1fr)]">
            ...
            <div class="space-y-8">
                <header class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <p class="text-sm text-on-surface-variant">Admin • Modifier le quiz</p>
                        <h1 class="text-3xl font-headline font-extrabold text-on-surface">Modifier {{ $quiz->title }}</h1>
                    </div>
                    <div class="flex flex-wrap gap-3">
                        ...
                        <a href="{{ route('admin.quizzes.questions.index', $quiz) }}" class="inline-flex items-center gap-2 rounded-full border border-primary/10 bg-primary/5 px-5 py-3 text-sm font-semibold text-primary hover:bg-primary/10 transition">
                            <span class="material-symbols-outlined">quiz</span>
                            Gérer les questions
                        </a>
                    </div>
                ...
                <div class="rounded-3xl bg-white p-8 shadow-sm border border-surface-container">
                    <form action="{{ route('admin.quizzes.update', $quiz) }}" method="POST" class="space-y-6">
                        ...
                        <div>
                            ...
                            <input id="duration_minutes" name="duration_minutes" type="number" min="1" max="240" value="{{ old('duration_minutes', $quiz->duration_minutes) }}" required
                                class="mt-2 w-full rounded-2xl border border-surface-container bg-surface-container-lowest px-4 py-3 text-on-surface outline-none focus:border-primary focus:ring-2 focus:ring-primary/10" />
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="5" class="mt-2 w-full rounded-2xl border border-surface-container bg-surface-container-lowest px-4 py-3 text-on-surface outline-none focus:border-primary focus:ring-2 focus:ring-primary/10">{{ old('description', $quiz->description) }}</textarea>
                            @error('description')
                                <p class="mt-2 text-sm text-error">{{ $message }}</p>
                            @enderror
                        </div>
